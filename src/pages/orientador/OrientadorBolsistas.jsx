...
import { Plus, ChevronDown, ChevronUp, Upload, FileText, CheckCircle, AlertTriangle, Trash2 } from 'lucide-react'
...
function DocUploadField({ label, reference, fieldKey, currentUrl, onUpload, uploading }) {
  const ref = useRef()
  ...
  return (
    <div className="space-y-0.5">
      ...
      <div
        className={`border rounded-lg p-2.5 flex items-center gap-2 cursor-pointer transition-colors ${
          hasFile
      ...
      >
        {hasFile ? (
          ...
          <>
            ...
            <span className="text-xs text-gray-400">Enviar (PDF, JPG, PNG — máx. 5MB)</span>
          </>
        )}
      </div>
      <input
...
function BolsistaCard({ bolsista, projeto, expanded, onToggle, onUpdate, onDelete }) {
  ...
  function handleChange(e) {
    ...
    const v = name === 'cpf' ? maskCpf(value) : value
    setForm(prev => ({ ...prev, [name]: v }))
  }

  async function handleBlur(e) {
    const { name, value } = e.target
    if (!bolsista.id || !value) return
    const { error } = await supabase.from('bolsista').update({ [name]: value }).eq('id', bolsista.id)
    if (!error) onUpdate({ ...bolsista, [name]: value })
  }

  async function handleUpload(fieldKey, file) {
    if (!bolsista.id) { setFieldError('Salve o nome antes de enviar documentos.'); return }
    setUploading(prev => ({ ...prev, [fieldKey]: true }))
    setFieldError(null)
    try {
      const url = await uploadArquivo(file, projeto.id, bolsista.id, fieldKey)
      const { error } = await supabase.from('bolsista').update({ [fieldKey]: url }).eq('id', bolsista.id)
      if (!error) onUpdate({ ...bolsista, [fieldKey]: url })
    } catch (err) {
  ...
  async function handleDelete() {
    if (!bolsista.id) { onDelete(bolsista.id); return }
    if (!window.confirm(`Remover ${bolsista.nome_completo || 'este bolsista'} da equipe?`)) return
    setDeleting(true)
    await supabase.from('bolsista').update({ status: 'inativo' }).eq('id', bolsista.id)
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-900 truncate">
            {form.nome_completo || <span className="text-gray-400 font-normal">Novo bolsista</span>}
          </p>
          <p className="text-xs text-gray-400">
            {form.ano_escolar || '—'}
            {form.data_nascimento && ` · ${calcIdade(form.data_nascimento)} anos`}
          </p>
        </div>
...
export function OrientadorBolsistas() {
  ...
  async function handleAdicionar() {
    if (!projeto || bolsistas.length >= MAX_BOLSISTAS) return
    setAdicionando(true)
    setError(null)
    const { data, error: err } = await supabase
      .from('bolsista')
      .insert({ projeto_id: projeto.id, orientador_id: orientador.id, status: 'ativo' })
      .select()
      .single()
    setAdicionando(false)
    if (err) { setError('Erro ao criar bolsista. Tente novamente.'); return }
    setBolsistas(prev => [...prev, data])
    setExpanded(data.id)
  }
      ...
      <main className="flex-1 ml-[200px] p-6 space-y-5" style={{ maxWidth: 'calc(100% - 200px)' }}>
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={adicionando || bolsistas.length >= MAX_BOLSISTAS || loading}
          ...
          >
            ...
            {adicionando ? 'Aguarde...' : 'Cadastrar novo bolsista'}
          </button>
        </div>
