...
export const CATEGORIAS_ATA_CMCT = [
  { value: 'ata', label: 'Ata' },
...
// Upload + cadastro de um anexo de reunião do CMCT em documento_acervo
// (entidade_tipo='reuniao_cmct'). Segue o mesmo padrão de AnexarDocumento.jsx,
// mas com categorias e path de storage próprios do módulo de Atas.
export function AnexarDocumentoAtaCmct({ reuniaoId, onUploaded, label = 'Anexar arquivo' }) {
  ...
  async function handleEnviar() {
    if (!file || !reuniaoId) return
    setUploading(true)
    setError(null)
    try {
      const path = `atas-cmct/${reuniaoId}/${Date.now()}-${sanitizarNomeArquivo(file.name)}`
      const { error: upErr } = await supabase.storage.from(BUCKET).upload(path, file)
      if (upErr) throw upErr
      const { data: { publicUrl } } = supabase.storage.from(BUCKET).getPublicUrl(path)
      const doc = await documentoAcervoService.create({
        entidade_tipo: 'reuniao_cmct',
      ...
      {file && (
        <div className="mt-2 p-3 border border-border rounded-md bg-muted/30 space-y-2 max-w-sm">
          ...
          <Select value={categoria} onChange={(e) => setCategoria(e.target.value)}>
            {CATEGORIAS_ATA_CMCT.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
          </Select>
          <input
            type="text"
